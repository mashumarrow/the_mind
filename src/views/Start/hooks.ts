...
export const ChangeNowCard = async (id : string, card : number | null) => {
  const { data, error } = await supabase.from("rooms").update({ now_card: card}).eq("RoomID", id).select('now_card');
  if (error) {
...
export const UpdateSuccess = async (RoomID : string, posiblity : boolean | null) => {
  if (posiblity === null) return console.log('成功可能性がnullです。');
    const {error} = await supabase.from("rooms").update({ success: posiblity }).eq("RoomID", RoomID);
    if (error) {
